...
Create the next 24-hour audit note, reread the full completed manuscript, then make exactly one count-flat prose revision or light compression based on the weakest passage. The draft is currently 22,418 words / 89.7 pages, below but close to the 22,500-word / 90-page compression trigger, so do not add new scenes unless replacing weaker material. Candidate next targets are Chapter 7 route-scene tightening or a style-level Chapter 1 pass. Preserve the lawful margin note, Maura Dain's name, Tomas's unresolved bond cost, custody limits, Aurelith absence, and the quiet thin-veil ending.
